...
export function RequireAdmin() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6 dark:bg-neutral-950">
        <div className="w-full max-w-sm space-y-3" aria-hidden="true">
          <div className="h-10 animate-pulse rounded-xl bg-slate-200 dark:bg-neutral-800" />
  ...
  if (state.kind === 'unreachable') {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6 dark:bg-neutral-950">
        <div className="max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm anim-auf dark:border-white/10 dark:bg-neutral-900">
          <div className="mx-auto mb-4 flex w-16 h-16 items-center justify-center rounded-3xl bg-slate-100 dark:bg-neutral-800">
            <ServerCrash className="w-8 h-8 text-slate-500 dark:text-neutral-400" aria-hidden="true" />
          </div>
          <h2 className="mb-2 text-2xl font-bold text-slate-800 dark:text-neutral-100">Der Server antwortet nicht</h2>
          <p className="text-slate-600 dark:text-neutral-400">{state.message}</p>
          <button
            type="button"
            onClick={recheck}
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 font-semibold text-white shadow-lg shadow-indigo-200 transition-all hover:bg-indigo-700 hover:-translate-y-0.5 active:translate-y-0 dark:shadow-black/30"
          >
