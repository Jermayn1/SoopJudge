...
export function ConfirmDialog({
  ...
  return (
    <dialog
      ...
      className="m-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-0 shadow-2xl backdrop:bg-slate-900/40 dark:border-white/10 dark:bg-neutral-900 dark:backdrop:bg-black/60"
    >
      <div className="p-6">
        <div className="flex gap-4">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-amber-50 ring-1 ring-amber-200 dark:bg-amber-500/10 dark:ring-amber-400/30">
            <AlertTriangle className="w-5 h-5 text-amber-900 dark:text-amber-200" aria-hidden="true" />
          </div>
          <div className="min-w-0">
            <h2 id={titleId} className="text-lg font-bold text-slate-800 dark:text-neutral-100">
              {title}
            </h2>
            <p className="mt-1 text-sm text-slate-600 dark:text-neutral-400">{message}</p>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            ...
            onClick={onCancel}
            disabled={busy}
            className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-white/15 dark:text-neutral-300 dark:hover:bg-neutral-800"
          >
            Abbrechen
          </button>
          <button
